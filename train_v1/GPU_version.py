...
with tf.Graph().as_default(), tf.Session(config=config) as sess:
    ...
    phone_image_before = tf.reshape(phone_, [-1, PATCH_HEIGHT, PATCH_WIDTH, 3])
    phone_image = tf.split(value=phone_image_before, num_or_size_splits=GPU_NUM, axis=0)

    dslr_ = tf.placeholder(tf.float32, [None, PATCH_SIZE])
    dslr_image_before = tf.reshape(dslr_, [-1, PATCH_HEIGHT, PATCH_WIDTH, 3])
    dslr_image = tf.split(value=dslr_image_before, num_or_size_splits=GPU_NUM, axis=0)

    adv = tf.placeholder(tf.float32, [None, 1])
    adv_ = tf.split(value=adv,num_or_size_splits=GPU_NUM,axis=0)
    loss_color1 = []
    loss_content1 = []
    loss_discrim1 = []
    loss_generator1 = []
    ...
    for i in range(0, GPU_NUM):
        with tf.device("/gpu:{}".format(i)), tf.variable_scope(name_or_scope=tf.get_variable_scope(),
            ...
            dslr_gray = tf.reshape(tf.image.rgb_to_grayscale(dslr_image[i]), [-1, PATCH_WIDTH * PATCH_HEIGHT])

            # push randomly the enhanced or dslr image to an adversarial CNN-discriminator

            adversarial_ = tf.multiply(enhanced_gray, 1 - adv_[i]) + tf.multiply(dslr_gray, adv_[i])
            ...
            discrim_target = tf.concat([adv_[i], 1 - adv_[i]], 1)
            ...
            content_size = utils._tensor_size(dslr_vgg[CONTENT_LAYER]) * batch_size/GPU_NUM
            ...
            loss_color = tf.reduce_sum(tf.pow(dslr_blur - enhanced_blur, 2)) / (2 * batch_size/GPU_NUM)
            ...
            loss_tv = 2 * (x_tv / tv_x_size + y_tv / tv_y_size) / batch_size/GPU_NUM
            ...
            dslr_image_flat = tf.reshape(dslr_image[i],[-1, PATCH_SIZE])
            loss_mse = tf.reduce_sum(tf.pow(dslr_image_flat - enhanced_flat, 2)) / (PATCH_SIZE * batch_size/GPU_NUM)
            loss_psnr = 20 * utils.log10(1.0 / tf.sqrt(loss_mse))

            loss_color1.append(loss_color)
            loss_content1.append(loss_content)
            loss_discrim1.append(loss_discrim)
            loss_tv1.append(loss_tv)
            loss_texture1.append(loss_texture)
            loss_mse1.append(loss_mse)
            loss_psnr1.append(loss_psnr)
            loss_generator1.append(loss_generator)
            discim_accuracy1.append(discim_accuracy)
            if i==0:
                enhanced1 = enhanced
            else:
                enhanced1 = tf.concat([enhanced1, enhanced], 0)


    loss_generator = tf.reduce_mean(loss_generator1)
    loss_psnr = tf.reduce_mean(loss_psnr1)
    loss_color = tf.reduce_mean(loss_color1)
    loss_content = tf.reduce_mean(loss_content1)
    loss_texture = tf.reduce_mean(loss_texture1)
    loss_mse = tf.reduce_mean(loss_mse1)
    loss_discrim = tf.reduce_mean(loss_discrim1)
    loss_tv = tf.reduce_mean(loss_tv1)
    discim_accuracy = tf.reduce_mean(discim_accuracy1)
    ...
    test_crops = test_data[np.random.randint(0, TEST_SIZE, 4), :]

    logs = open('/gdata/huangjie/DPED/models/' + phone + '.txt', "w+")
    logs.close()

    saver.restore(sess, "/gdata/huangjie/DPED/models/" + phone + "_iteration_" + "32600" + ".ckpt")

    for i in range(32600,32600+num_train_iters):
        ...
        [loss_temp, temp] = sess.run([loss_generator, train_step_gen],
                                     feed_dict={phone_: phone_images, dslr_: dslr_images, adv: all_zeros})
        ...
        [accuracy_temp, temp] = sess.run([discim_accuracy, train_step_disc],
                                         feed_dict={phone_: phone_images, dslr_: dslr_images, adv: swaps})
        ...
        if i % eval_step == 0:
            ...
            for j in range(num_test_batches):
                ...
                [enhanced_crops, accuracy_disc, losses] = sess.run([enhanced1, discim_accuracy, \
            ...
            logs = open('/gdata/huangjie/DPED/models/' + phone + '.txt', "a")
            ...
            enhanced_crops = sess.run(enhanced1, feed_dict={phone_: test_crops, dslr_: dslr_images, adv: all_zeros})

            idx = 0
            for crop in enhanced_crops:
                before_after = np.hstack((np.reshape(test_crops[idx], [PATCH_HEIGHT, PATCH_WIDTH, 3]), crop))
                misc.imsave(
                    '/gdata/huangjie/DPED/results/' + str(phone) + "_" + str(idx) + '_iteration_' + str(i) + '.jpg',
                    before_after)
                idx += 1
            ...
            saver.save(sess, '/gdata/huangjie/DPED/models/' + str(phone) + '_iteration_' + str(i) + '.ckpt',
