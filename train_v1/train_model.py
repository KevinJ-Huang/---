...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    loss_texture = tf.reduce_sum(tf.pow(tf.image.rgb_to_grayscale(dslr_image) - tf.image.rgb_to_grayscale(enhanced), 2)) / (2 * batch_size)





    # 1) texture (compute_gram) loss

    # enhanced_vgg = vgg.net(vgg_dir, vgg.preprocess(enhanced * 255))
    # dslr_vgg = vgg.net(vgg_dir, vgg.preprocess(dslr_image * 255))
    # enhanced_features = vgg.get_features(enhanced_vgg)
    # dslr_features = vgg.get_features(dslr_vgg)
    # enhanced_list = compute_gram(enhanced_features)
    # dslr_list = compute_gram(dslr_features)
    # loss_list = [ ]
    # for j in range(0, len(enhanced_list)):
    #     shape = tf.shape(enhanced_list[j])
    #     feature_size = tf.cast(shape[1] * shape[2], tf.float32)
    #     layer_loss = tf.reduce_sum((enhanced_list[j] - dslr_list[j]) ** 2) / (feature_size*batch_size)
    #     loss_list.append(layer_loss)
    # loss_texture = tf.reduce_mean(loss_list)
    ...
    loss_color = tf.reduce_sum(tf.pow(dslr_image - enhanced, 2)) / (2 * batch_size)  #elimilate Gaussian blur
    ...
    # loss_generator =  w_texture * loss_texture + w_color * loss_color + w_tv * loss_tv
    # psnr loss
    ...
    logs = open('/gdata/huangjie/DPED/models2/' + phone + '.txt', "w+")
    logs.close()

    # saver.restore(sess, "/gdata/huangjie/DPED/models1/" + phone + "_iteration_" + "100000" + ".ckpt")
    ...
    for i in range(0, num_train_iters):
        ...
        if i % eval_step == 0:
            ...
            logs = open('/gdata/huangjie/DPED/models2/' + phone + '.txt', "a")
            ...
            for crop in enhanced_crops:
                ...
                misc.imsave('/gdata/huangjie/DPED/results2/' + str(phone) + "_" + str(idx) + '_iteration_' + str(i) + '.jpg',
                            before_after)
                idx += 1
            ...
            saver.save(sess, '/gdata/huangjie/DPED/models2/' + str(phone) + '_iteration_' + str(i) + '.ckpt',
