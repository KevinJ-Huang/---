...
def load_test_data(phone, dped_dir, IMAGE_SIZE):
    ...
    for i in range(0, NUM_TEST_IMAGES):
        if os.path.exists(test_directory_phone + str(i) + '.jpg'):
            ...
            I = np.asarray(misc.imread(test_directory_phone + str(i%1500) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
            test_data[i, :] = I

            I = np.asarray(misc.imread(test_directory_dslr + str(i%1500) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
            test_answ[i, :] = I
...
def load_batch(phone, dped_dir, TRAIN_SIZE, IMAGE_SIZE):
    ...
    for img in TRAIN_IMAGES:
        if os.path.exists(train_directory_phone + str(img) + '.jpg'):
            ...
            I = np.asarray(misc.imread(train_directory_phone + str(img%1500) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
            train_data[i, :] = I

            I = np.asarray(misc.imread(train_directory_dslr + str(img%1500) + '.jpg'))
            I = np.float16(np.reshape(I, [1, IMAGE_SIZE])) / 255
            train_answ[i, :] = I
