...
def log_hook(sess, log_fetches):
  ...
  loss_filter=data['loss_filter']
  loss_texture=data['loss_texture']
  loss_tv=data['loss_tv']
  discim_accuracy = data['discim_accuracy']
  discim_accuracy1 = data['discim_accuracy1']
  psnr = data['psnr']
  loss_ssim = data['loss_ssim']
  log.info('Step {} | loss = {:.4f}| loss_content = {:.4f} | '
'loss_color = {:.4f} | loss_filter = {:.4f} |loss_texture = {:.4f} | loss_tv = {:.6f} |discim_accuracy =  {:.4f} |discim_accuracy1 =  {:.4f}| psnr = {:.2f} dB|ssim = {:.4f}'.format(step,
                                          loss, loss_content, loss_color, loss_filter, loss_texture, loss_tv, discim_accuracy, discim_accuracy1, psnr, loss_ssim))
...
if __name__ == '__main__':
  ...
  req_grp.add_argument('--checkpoint_dir', default='/gdata/huangjie/hdrnet/hdrnet-loss/checkpoint13/', help='directory to save checkpoints to.')
  req_grp.add_argument('--data_dir', default= '/gdata/huangjie/hdrnet/sample_data/identity/filelist.txt', help='input directory containing the training .tfrecords or images.')
  ...
  train_grp.add_argument('--eval_interval', type=int, default=100, help='interval between evaluations (in s)')
  ...
  data_grp.add_argument('--scale', default= 2, type=int, help='resolution scale of the low image.')
